...
import { Box, Heading, SimpleGrid, Stat, StatLabel, StatNumber, Progress, Text, useColorModeValue } from "@chakra-ui/react";
...
export default function DashboardOverview(){
  ...
  useEffect(() => {
    ...
    adminService.getListingsLast7Days().then((res) => {
  ...
  return (
    <Box>
      ...
      <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>
        <Box>
          <Heading size="sm" mb={2}>Người dùng mới (7 ngày gần nhất)</Heading>
          <LineChartComponent data={userSeries} />
        </Box>
        <Box>
          <Heading size="sm" mb={2}>Tin mới (7 ngày gần nhất)</Heading>
