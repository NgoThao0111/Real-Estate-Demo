...
const SlideShow = ({ listings = [] }) => {
  ...
  const formatPrice = (price, rentalType) => {
    return price
      ? `${Number(price).toLocaleString("vi-VN")} ${rentalType === "rent" ? "VNĐ/tháng" : "VNĐ"}`
      : "Liên hệ";
  };
    ...
    return (
      ...
      >
        ...
        >
          <Center>
            <VStack spacing={3}>
              <Heading color={useColorModeValue("gray.800", "white")} fontSize={{ base: "2xl", md: "4xl" }}>
                Tìm ngôi nhà mơ ước của bạn
              </Heading>
              <Text color={useColorModeValue("gray.600", "gray.400")}>
                Không có tin đăng mới nào để hiển thị
              </Text>
            </VStack>
          </Center>
        </Box>
    ...
    >
      ...
      >
        <VStack align="start" spacing={3}>
          ...
          >
            {formatPrice(recentListings[currentSlide]?.price, recentListings[currentSlide]?.rental_type)}
          </Text>
          <Text
